...
**PortPay is Mainnet-first; the buyer-signed Pay handoff is implemented but real Mainnet use awaits final review.** Normal merchant, buyer, receipt, and history routes use X Layer Mainnet (chain `196`) without a query selector or network switch. Buyer checkout sizes the selected supported xStock to the invoice. If exact allowance is missing, the buyer can separately confirm the prepared exact approval; Pay is available only after the allowance reread and same-preparation backend recheck return READY. The connected buyer manually confirms the exact swap in OKX Wallet. No replacement swap is fetched after READY and nothing is sent automatically. The backend never signs or broadcasts, and an invoice becomes paid only after canonical reconciliation.
...
The server-side adapter uses the authenticated OKX V6 DEX API at its pinned official origin for quotes, exact approval calldata, and swap transaction preparation. It validates chain `196`, independently verified xStock and USD₮0 addresses, decoded invoice merchant recipient, decoded tokens and amounts, quote freshness, minimum receive amount, router, spender, and exact approval amount. Unsupported router call shapes fail closed. The backend approval-preparation endpoint uses the existing immutable Supabase evidence and full Phase 2 preflight; it never accepts caller-supplied spender, amount, calldata, or swap data as authority. Production checkout has no fixed proof amount: it uses at most four bounded fresh exact-in sizing quotes plus one final validation quote (five quote requests total) to size the buyer-selected wNVDAx or wAAPLx input to the invoice, then creates one authenticated persisted `/swap` preparation. The protected quote output and executable minimum receive must cover the pending invoice.

The ordinary `/pay/:invoiceId` route is Mainnet by default; there is no network query selector in normal product routes. The buyer connects OKX Wallet on chain `196` and selects wNVDAx or wAAPLx. Checkout requires the exact allowance for that invoice-sized input; when it is missing, the buyer may separately confirm one backend-prepared exact approval and checkout rereads the allowance before enabling Pay. Pay itself never requests approval. Preparation expiry is capped at the earlier of `preparedAt + 120 seconds` and the deadline encoded in that exact OKX calldata; the preview quote must still be fresh when the swap response is accepted. Neither displayed expiry nor calldata is extended. Once checkout reaches READY, Pay never replaces the preparation or requests another `/swap`: with at least 30 seconds remaining, it rechecks that same persisted preparation. If fewer than 30 seconds remain before handoff, checkout returns to the preparation stage instead of continuing Pay; the buyer explicitly refreshes, reviews the new amount and allowance, and clicks Pay again. Any newly required approval is evaluated against that refreshed input. The read-only readiness check also requires at least 30 seconds remaining after its checks; if that cannot be met, no wallet prompt or handoff occurs and the invoice remains retryable. The buyer then signs the bound handoff authorization message; the backend repeats the checks and persists the handoff only while still unexpired. Only a successfully persisted handoff starts the one-attempt rule. The subsequent READY response supplies matching persisted transaction fields, which the frontend passes unchanged to OKX Wallet. The buyer manually confirms the swap.
...
READY returns the same persisted preparation after the exact allowance passes Stage B. If allowance is missing, checkout offers the backend-prepared exact amount for that invoice and submits only the attributed approval calldata after the buyer manually confirms it. A successful receipt is followed by an onchain exact-allowance reread and a server recheck of the same persisted preparation; no new quote or `/swap` is requested during this transition. Pay itself requires that exact allowance and remains approval-free. Before asking the buyer to sign the message bound to the invoice, preparation, calldata, buyer, and expiry, checkout performs its non-consuming read-only final readiness check. With at least 30 seconds remaining, Pay keeps and rechecks the exact READY preparation. Below 30 seconds, Pay returns to the preparation stage and does not prompt or silently replace-and-continue; a fresh preparation is obtained there, its exact input/allowance state is reviewed, and Pay requires another explicit click. Message-signature cancellation, expiry before handoff, or failed handoff creation with no persisted row does not consume the invoice attempt. The buyer separately confirms the exact transaction in the wallet. Each invoice permits only one Mainnet attempt after a handoff is successfully persisted; after that, no second swap prompt is allowed, even after transaction rejection or lost result. A recorded transaction hash can only be recovered and canonically reconciled. If no hash is recorded after a persisted handoff, checkout shows **Payment status unresolved** after reload; the merchant must create a new invoice for another attempt after checking the buyer's wallet. Browser storage cannot authorize another swap.
...
Before the wallet message prompt, `POST /api/invoices/:invoiceId/mainnet/readiness-recheck` accepts a persisted preparation ID, buyer address, and `preflightOnly: true`. Pay keeps a READY preparation whenever at least 30 seconds remain and runs final readiness against that same persisted preparation; it does not request another quote or `/swap`. If fewer than 30 seconds remain before handoff, Pay returns to the preparation stage without a wallet prompt or handoff. The buyer explicitly refreshes there; the refreshed amount and exact allowance are re-evaluated, and Pay requires another explicit click. The endpoint reloads immutable evidence, verifies its hash and invoice/chain/buyer/Builder Code bindings, checks current exact allowance and balances, estimates gas, and calls the exact persisted attributed calldata. It returns no wallet transaction and creates no handoff; it requires at least 30 seconds of remaining preparation lifetime after checks. Preparation expiry remains capped by the encoded OKX deadline and is never extended. After the buyer signs the bound authorization message, the same endpoint is called with the signature; it repeats all checks and the database records the handoff only if still unexpired. If no handoff row was persisted, expiry/failure does not consume the invoice attempt. Only a successfully persisted handoff returns the exact prepared transaction fields needed for the subsequent manual wallet confirmation. No further OKX `/swap` call occurs during either readiness recheck, and calldata is never mutated. `POST /api/invoices/:invoiceId/mainnet/submitted` accepts preparation ID, handoff ID, and transaction hash; it records the exact RPC-observed transaction, even if observation is after expiry, only when linked to that valid pre-expiry handoff. `POST /api/invoices/:invoiceId/mainnet/reconcile` accepts preparation ID and transaction hash, derives all payment fields from persisted server evidence, invokes the canonical verifier, and finalizes settlement plus invoice paid state atomically. A paid retry succeeds only after the same mainnet network, preparation, transaction, and evidence are reverified. The backend does not sign or broadcast. Real Mainnet use must wait for final GPT-5.6 Sol High review and manual buyer wallet confirmation.
...
- `POST /api/invoices/<uuid>/mainnet/approval-preparation` returns invoice-sized preparation data after authenticated backend preparation/preflight. When allowance is missing, it also returns the exact backend-prepared approval; after manual wallet confirmation, the same preparation can be rechecked by ID without requesting another swap.
...
4. Connect the buyer OKX Wallet on chain 196 and select wNVDAx or wAAPLx. If the exact invoice-sized allowance is missing, confirm the separately presented exact approval in the wallet; checkout rereads allowance before showing Pay. Pay itself does not approve tokens.
...
- Mainnet wallet submission is available only through the manual Pay handoff after same-calldata readiness recheck. Checkout may request one separately confirmed, exact invoice-sized approval when needed; Pay itself is approval-free. No automatic send or backend signing/broadcast exists. Real Mainnet use requires final GPT-5.6 Sol High review and manual wallet confirmation.
