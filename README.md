...
The ordinary `/pay/:invoiceId` route is Mainnet by default; there is no network query selector in normal product routes. The buyer connects OKX Wallet on chain `196`; checkout uses the existing persisted preparation and requires the exact wNVDAx allowance to already exist. On an explicit Pay click, the buyer signs a message authorizing this one handoff. The backend verifies it and rechecks that same preparation, including invoice, buyer, chain, Builder Code, balances, exact allowance, expiry, gas, and exact attributed calldata simulations. Only a fresh READY response with matching preparation bindings opens the swap transaction prompt. The frontend passes the returned `from`, `to`, exact `data`, `value`, and chain unchanged to OKX Wallet. No new approval is requested, and the buyer manually confirms the swap.
...
READY returns the same persisted preparation after the exact existing allowance passes Stage B. The Pay flow requires that exact allowance and does not offer or request a new token approval; any approval setup is separate from Pay. On Pay, the buyer first signs a message bound to that invoice, preparation, calldata, buyer, and expiry before the server reserves the wallet handoff; this signature does not submit payment. The buyer then separately confirms the exact transaction in the wallet. Each invoice permits only one Mainnet handoff and one swap wallet prompt. Once a handoff exists, Pay is never offered again for that invoice, even after wallet rejection or a lost result. A recorded transaction hash can only be recovered and canonically reconciled. If no hash is recorded, checkout shows **Payment status unresolved** after reload; the merchant must create a new invoice for another attempt after checking the buyer's wallet. Browser storage cannot authorize another swap.
