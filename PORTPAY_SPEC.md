...
This product default does **not** mean mainnet payments are fully live. Checkout sizes the buyer-selected supported xStock input from the invoice using bounded fresh exact-in quotes (exact-out is unsupported), then persists one final authenticated preparation. If the required exact allowance is missing, checkout may offer a separate manually confirmed, backend-prepared exact approval and must verify its successful receipt and reread the exact allowance before enabling Pay. The buyer-signed Pay handoff submits only the exact persisted swap after a fresh same-calldata server readiness recheck and explicit buyer wallet confirmation; Pay itself does not request approval or send automatically. The invoice-level handoff uniqueness blocker is cleared by the successfully applied and verified live migration `20260924000000_mainnet_handoff_invoice_unique.sql`. Real Mainnet use must still wait for final GPT-5.6 Sol High review and separate user authorization; only verified backend reconciliation may mark an invoice paid. Backend signing/broadcast remains prohibited. No real Mainnet swap has been sent.
...
4. **Buyer checkout**
   ...
   - If needed, explicitly approve the exact backend-prepared amount first; checkout waits for confirmation and rereads allowance. The manual Pay action then reuses that preparation and is available only after exact allowance and a fresh server readiness recheck return READY. No payment is confirmed until canonical backend reconciliation succeeds.
...
The Mainnet checkout may expose a separate exact-approval action before Pay. `POST /api/invoices/:invoiceId/mainnet/approval-preparation` accepts the connected buyer address, reads the pending invoice, and uses the buyer-selected supported xStock. It obtains fresh exact-in quotes and performs only a small bounded sizing search for an input whose protected minimum receive covers the invoice; exact-out is unsupported. The total quote-request budget is at most five: up to four sizing/refinement quotes plus one final validation quote. It then creates one authenticated OKX swap preparation and derives the exact approval amount, spender, and attributed calldata from that same preparation. The selected amount and preparation are persisted and bound through approval, preflight, Pay, and receipt verification. Client-supplied amount, token, spender, quote, or calldata are never authority. If allowance is not exact, Stage A simulates only the exact approval and returns `APPROVAL_REQUIRED`; the buyer manually confirms that approval, checkout waits for a successful receipt and rereads exact allowance, then reruns the remaining preflight gates against the same persisted swap preparation before exposing Pay. It must not replace that preparation or request another `/swap`. If allowance is already exact, approval is skipped. No unlimited/reusable approval is allowed. Pay itself never sends an approval. Testnet remains inaccessible from normal production routes.
...
Production Mainnet checkout has no fixed `$1.00` invoice or `0.0048 wNVDAx` input assumption. The exact xStock input is sized for the real invoice amount from fresh exact-in quotes using a small bounded process; the final authenticated swap preparation is created once for the selected input. Keep the `1.5%` slippage cap and require final `minimumReceive` to be at least the actual invoice amount. A preparation is `READY` only with exact pinned allowance, fresh authenticated preparation, verified direct merchant receiver and Builder Code, successful exact attributed gas estimation and `eth_call`, and sufficient OKB. If allowance is not exact, checkout exposes only the exact backend-prepared approval as a separate manual action; successful receipt and exact allowance reread are required before Pay. Wallet confirmation remains manual; no automatic swap broadcast is enabled. The local read-only proof runner may retain a fixed synthetic `$1.00` invoice/input solely as a diagnostic fixture, not as production sizing policy.
...
The pre-prompt read-only readiness check and the signed handoff recheck are separate steps over one final persisted preparation. READY remains bound to that persisted preparation. When Pay is clicked with at least 30 seconds remaining, checkout rechecks that same preparation and does not call `/swap`. If fewer than 30 seconds remain before handoff, Pay returns checkout to preparation without prompting; a fresh preparation is obtained there, its changed input and exact allowance are reevaluated, and the buyer must explicitly click Pay again. Preparation expiry is the earlier of `preparedAt + 120 seconds` and the deadline embedded in OKX calldata; the preview quote must be fresh when the final authenticated response is accepted. When the onchain deadline is shorter, the preparation is used just-in-time and is never presented as valid beyond the encoded deadline. The pre-prompt check revalidates its hash and invoice/buyer/chain/Builder Code bindings, pending status, fresh balances and exact allowance, then re-estimates/re-simulates the exact persisted attributed calldata. It returns no wallet transaction and creates no handoff; it requires at least 30 seconds of remaining preparation lifetime after checks. If the readiness check cannot meet that minimum, checkout returns to preparation before any wallet prompt; the invoice remains retryable and no handoff exists. If the same preparation requires approval, the buyer first submits its exact backend-prepared approval manually, and checkout verifies success and rereads allowance before allowing Pay. After the authorization message is signed, the second recheck repeats all gates and inserts the immutable handoff only while the preparation is strictly unexpired; the database insert has no arbitrary seconds buffer. If expiry wins during insertion and no handoff row is persisted, the invoice remains retryable with a fresh preparation. Only after the successful persisted handoff does the one-invoice/one-attempt rule begin. The response then returns the persisted wallet transaction fields and identifiers, which the frontend passes unchanged to the connected wallet. A later exact transaction observation is allowed only when bound to that pre-expiry authorization. An unused/expired preparation cannot create a new handoff. A retry for an already paid invoice is idempotent only after confirming mainnet payment network and the exact same preparation, transaction, and verified evidence; it must not accept a testnet-paid invoice or caller-supplied substituted evidence. Neither recheck calls `/swap`, replaces or mutates calldata, signs, or broadcasts. The buyer must confirm manually; Pay itself requests no approval. Real Mainnet use must wait for final GPT-5.6 Sol High review.
...
No mainnet wallet funding, Builder Code registration transaction, deployment, private-key execution, backend transaction-send API, or backend broadcast is permitted in this implementation. The buyer-signed Pay handoff is implemented: checkout may first request the exact approval from the same persisted invoice-sized preparation when allowance is missing; after successful receipt verification and exact allowance reread, the buyer may submit only the persisted, rechecked swap through manual wallet confirmation. Pay itself has no approval action or automatic send. Do not conduct a real Mainnet payment before final Sol High review. Approval is not payment, and only verified canonical reconciliation marks an invoice paid.
...
- Keep backend signing, backend transaction-send endpoints, and broadcasts disabled. The manual buyer-signed Pay handoff may submit only the exact persisted preparation after a fresh same-calldata recheck and explicit wallet confirmation; the Pay handoff itself does not request approval. A separate exact approval may be manually confirmed before Pay when required. Real Mainnet use remains held until final review and explicit user authorization. Readiness/submission/reconciliation endpoints do not sign or broadcast.
...
- Target 120 seconds from the final authenticated swap preparation: executable expiry is `min(preparedAt + 120 seconds, deadline encoded in OKX calldata)`, while the preview quote must still be fresh when accepted. Never extend displayed expiry or calldata. READY binds checkout to one persisted preparation. At Pay click, if at least 30 seconds remain, keep it and recheck the same preparation without requesting another `/swap`. If fewer than 30 seconds remain, return to preparation before any wallet prompt; explicitly obtain a fresh preparation there, re-evaluate its exact input allowance, and require another explicit Pay click. If the OKX-encoded deadline is shorter than 120 seconds, use the result just-in-time without claiming a longer window. The read-only same-preparation readiness check requires at least 30 seconds remaining after checks; if that fails, return to preparation and keep the invoice retryable. After the buyer signs, repeat all gates and insert the immutable database-timestamped handoff only if still unexpired; the database insert has no fixed-seconds buffer. If insertion fails because expiry wins and no row exists, the invoice remains retryable with a fresh preparation. Only a successfully persisted handoff starts the one-invoice/one-attempt rule. A submission observed later is valid only for the exact persisted preparation/calldata bound to that pre-expiry handoff. Once the final preparation is selected, the rechecks never call `/swap` again or mutate/replace calldata; no client timestamp is trusted.
