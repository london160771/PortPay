...
This default does not imply that mainnet payments are fully live. Checkout sizes the buyer-selected supported xStock input from the invoice using bounded fresh exact-in quotes, then persists one final authenticated preparation. If the exact allowance for that preparation is missing, checkout may present the backend-prepared exact approval as a separate manual wallet action and must verify its receipt and reread the exact allowance before enabling Pay. The buyer-signed Pay handoff submits only the exact persisted swap after a fresh same-calldata server readiness recheck and explicit buyer wallet confirmation; Pay itself never sends an approval or automatically submits. The invoice-level handoff uniqueness blocker is cleared by the successfully applied and verified live migration `20260924000000_mainnet_handoff_invoice_unique.sql`. Do not use the flow for a real Mainnet payment before the final GPT-5.6 Sol High review passes and the user separately authorizes the payment; only verified backend reconciliation can mark an invoice paid. Backend signing/broadcast remains prohibited. No real Mainnet swap has been sent. Legacy testnet records must retain their correct network/explorer display.
...
- Production Mainnet input sizing is invoice-driven: use the buyer-selected supported xStock and a small bounded sequence of fresh exact-in quotes to find an input whose protected minimum receive covers the invoice; exact-out is unsupported and must not be attempted. The total quote-request budget is at most five: up to four sizing/refinement quotes plus one final validation quote. Persist exactly one final authenticated swap preparation for the selected input and bind that amount through approval, preflight, handoff, and reconciliation. No fixed `$1.00` / `0.0048 wNVDAx` proof assumption may remain in the production checkout path. Keep the `1.5%` slippage cap; final minimum receive must cover the actual invoice amount. Fail clearly when buyer balance or route liquidity cannot cover it. Surface Pay readiness only after exact allowance, fresh authenticated preparation, direct merchant receiver, verified Builder Code, exact attributed gas estimation and `eth_call`, and sufficient OKB all pass. If allowance is missing, show a separate exact backend-prepared approval action, wait for success, reread allowance, and only then expose Pay. Wallet confirmation stays manual and no automatic swap broadcast is enabled.
...
- The exact buyer-approval transaction is prepared from the same persisted invoice-sized preparation and may be presented as a separate manual checkout action only when the required allowance is missing. It must remain chain-196, backend-prepared, exact (never unlimited), attributed, manually confirmed, and followed by successful receipt verification plus an exact allowance reread. The Pay handoff itself requires that exact allowance, submits no approval, and fails closed otherwise; it may not create a replacement preparation or request another `/swap` after READY.
- Buyer-signed Mainnet Pay handoff: before any wallet message prompt, normal `/pay/:invoiceId` must run a read-only final readiness recheck for the existing preparation; it returns no transaction fields and creates no handoff. If successful, the buyer manually signs the bound authorization message and the signed recheck repeats the gates. Only after the handoff is persisted may READY return the minimal persisted transaction fields (`from`, `to`, exact attributed `data`, zero `value`, chain 196) plus preparation/hash/handoff identifiers. The frontend must submit those fields byte-for-byte through the connected wallet without another OKX `/swap`, calldata mutation, fallback, or approval request. The user confirms the transaction manually. After wallet submission, record its hash through the existing submission route and use existing canonical reconciliation; only that verifier/finalizer may mark paid. No private-key handling, backend signing/broadcast, or automatic wallet prompt/send is allowed. Do not use for a real Mainnet payment before final Sol High review.
- Target a 120-second Mainnet preparation TTL starting at the final authenticated swap preparation timestamp. The preview quote must remain fresh when that response is accepted. Cap executable preparation expiry at `min(preparedAt + TTL, deadline embedded in exact OKX calldata)`; never extend displayed expiry or mutate the encoded deadline. Once checkout reaches READY, Pay must stay bound to that exact persisted preparation and must never request a replacement `/swap`. At Pay click, if at least 30 seconds remain, keep it and run readiness against the same preparation. If fewer than 30 seconds remain, return to the preparation stage without a wallet prompt or silent refresh-and-continue; obtain a fresh preparation there, re-evaluate exact allowance for its new input, and require a separate Pay click. If the OKX deadline is shorter, the fresh response is used just-in-time and is not represented as a 120-second window. The read-only readiness check still requires at least 30 seconds remaining after its checks; if that floor is missed, return to preparation before any wallet prompt. This read-only check never creates a handoff or consumes the invoice attempt. The signed server recheck repeats all gates and the database still allows handoff insertion only while strictly unexpired (no 15/30-second insert buffer); if expiry wins during insertion and no handoff row is persisted, the invoice remains retryable with a fresh preparation. The successful database handoff insert is the one-invoice/one-attempt boundary. Later RPC observation is allowed only for the byte-identical transaction linked to a valid pre-expiry handoff, and client timestamps are never accepted.
...
- Verify a buyer-signed, non-transaction handoff message bound to the invoice, preparation and calldata hashes, buyer, chain, and expiry before reserving a handoff. The wallet transaction remains the separate payment-consent step. Enforce one handoff per invoice with the separate database-unique `mainnet_handoffs.invoice_id` index; retain the handoff UUID primary key and preparation uniqueness. Duplicate invoice lookup must fail closed. The successful persisted handoff is the single Mainnet payment attempt: after it exists, never return another swap wallet transaction for that invoice, regardless of transaction rejection, browser loss, or missing hash. Cancellation of the earlier authorization-message prompt, preparation expiry, or failed handoff insertion with no persisted row does not consume the attempt and may refresh/retry the same pending invoice. A recorded submission/hash can only be recovered and canonically reconciled; without one after a persisted handoff, show the payment as unresolved and require a new invoice for another attempt. Do not create another handoff after the first is persisted. During bounded invoice sizing, use fresh authenticated exact-in `/quote` requests; once input is selected, create one final authenticated `/swap` preparation. After checkout reaches READY, Pay must use that preparation: if at least 30 seconds remain, recheck it without another `/swap`; below 30 seconds, return to preparation before any wallet prompt, explicitly create/review the fresh preparation and allowance, and require another Pay click. Readiness/handoff rechecks do not call `/swap`. If approval is needed, it must occur before Pay and handoff persistence; never request approval after handoff creation. Browser storage cannot authorize another swap.
...
- The buyer-signed Mainnet Pay handoff is implemented as described above. Before the final Sol High review, do not conduct a real Mainnet payment. Preserve invoice-driven input sizing, the separate exact manual approval when needed, exact allowance before Pay, fresh same-calldata recheck, manual wallet confirmation, post-submit observation/reconciliation, and all verifier gates. Never add automatic submission or backend signing/broadcast.
